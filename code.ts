  ...
  if (msg.type === 'submit') {
    figma.loadAllPagesAsync().then(() => {
      ...
      for (const node of selectedNodes) {
        ...
        if ("findAll" in node) {
          const nestedTextNodes = node.findAll(n => n.type === "TEXT") as TextNode[];
      ...
      const figmaContent = new Map<string, string>();
      allTextNodes.map(node => 
        figmaContent.set(node.name, node.characters)
      );
      ...
      fetch("https://iaasexp-aml-workspace-aoai.openai.azure.com/openai/responses?api-version=2025-04-01-preview", {
        ...
        response.json().then(data => {
          ...
          const suggestedContent = new Map<string, string>();
